...
from extractor import DrivingLicenceExtractor

app = FastAPI(title="AI Driving Licence Scanner & Field Extractor")
...
JSON_STORE_PATH = os.path.join(BASE_DIR, "extracted_driving_licence_data.json")

os.makedirs(UPLOADS_DIR, exist_ok=True)
os.makedirs(PORTRAITS_DIR, exist_ok=True)
os.makedirs(STATIC_DIR, exist_ok=True)

extractor = DrivingLicenceExtractor(output_photo_dir=PORTRAITS_DIR)
...
async def serve_index():
    ...
    return HTMLResponse("<h1>Driving Licence Scanner App running. Static index.html not found.</h1>")

@app.post("/api/scan")
async def scan_dl_files(files: List[UploadFile] = File(...)):
    if not files:
    ...
    for file in files:
        ...
        file_name = file.filename or "uploaded_driving_licence.jpg"
...
async def scan_camera_snapshot(image_base64: str = Form(...), file_name: str = Form("dl_camera_capture.jpg")):
    if not image_base64:
...
async def export_json():
    ...
    return FileResponse(
        ...
        filename="extracted_driving_licence_data.json",
